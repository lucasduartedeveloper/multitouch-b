...
$(document).ready(function() {
    ...
    mic.onupdate = function(freqArray, reachedFreq, avgValue) {
        ...
        ambientAudio += micAvgValue;
        audioButtons[buttonNo].innerText = 
        ambientAudio.toFixed(2)+"%";

        if (micAvgValue >= micThreshold)
        thresholdReached = true;

        if (ambientAudio > 100) {
            audioButtons[buttonNo].style.fontSize = (sw/8)+"px";
            audioButtons[buttonNo].style.fontHeight = (sw/8)+"px";
            audioButtons[buttonNo].style.color = "#fff";
            audioButtons[buttonNo].innerText = 
            thresholdReached ? "✓" : "×";
            ...
            buttonNo = (buttonNo+1) < audioButtons.length ? 
            (buttonNo+1) : 0;
            for (var n = 0; n < audioButtons.length; n++) {
                audioButtons[n].style.backgroundColor = 
                backgroundColor;
            }

            audioButtons[buttonNo].style.backgroundColor = 
            "#fff";
            audioButtons[buttonNo].innerText = 
            ambientAudio.toFixed(2)+"%";
        }
    ...
    for (var x = 0; x < 4; x++) {
        ...
        audioButtonView.style.background = 
        backgroundColor;
    ...
    audioButtons[buttonNo].style.backgroundColor = 
    "#fff";
    audioButtons[buttonNo].innerText = "0%";
